...
export class SlackService {
  ...
  getSlackAuthorizationUrl(): string {
    return `https://slack.com/oauth/authorize?scope=identity.basic&scope=groups:read,channels:read,mpim:read,im:read&client_id=${dynamicEnv.slack.clientId}&redirect_uri=&state=&user_scope=&granular_bot_scope=0&team=&install_redirect=&single_channel=0&tracked=1&redirect_uri=${window.location.origin}`;
  }
